...
# SEO Cleanup 2026-07-20 (Session mit Claude/Cowork)

Kontext: GSC zeigte 231 nicht indexierte Seiten ("Crawled - currently not indexed": 220), nur /de/ und /en/ im Index. Ursache war KEIN technischer Fehler, sondern Googles Qualitätsfilter nach Bulk-Upload von 371 Blogartikeln im Mai + Duplicate Content (Root-Kopien, Subdomain).

## Was geändert wurde (alles committet + deployt am 20.07.2026)

1. **Blogs offline:** Alle 373 Blog-Dateien (`de/blog/`, `en/blog/`) nach `_reserve/` verschoben. `_reserve/` ist in `.gitignore` und existiert NUR lokal auf Marcels Mac. Blogs sind bewusst offline (Google-Quality-Reset). NICHT wieder live schalten ohne inhaltliche Überarbeitung; wenn einzelne zurückkommen, dann schrittweise (2-4/Woche), mit interner Verlinkung vom Blog-Index.
2. **Root-Seiten sind jetzt Redirect-Stubs:** `index.html`, `features.html`, `preis.html` etc. im Root sind kleine Meta-Refresh-Stubs (0s) mit Canonical auf das `/de/`-Pendant. GitHub Pages kann keine Server-Redirects, daher Stubs. NICHT durch echte Seiten ersetzen. `404.html` ist die echte 404-Seite, kein Stub.
3. **7 EN-Alt-Slug-Stubs:** `en/preis.html`→`en/pricing.html`, `en/ueber-uns.html`→`en/about.html`, `en/kontakt.html`→`en/contact.html`, `en/agb.html`→`en/terms.html`, `en/impressum.html`→`en/imprint.html`, `en/ki-tutor-guide-...`→`en/ai-tutor-guide-...`, `en/wann-lohnt-...`→`en/self-hosting-...`.
4. **layout.js:** Automatischer Sprachredirect beim Seitenladen ENTFERNT (war SEO-schädlich; leitete z. B. Googlebot und Nutzer mit localStorage `preferredLang` quer zwischen den Sprachen um). Der DE/EN-Toggle funktioniert weiterhin. Nicht wieder einbauen.
5. **sitemap.xml:** 420 → 48 kanonische URLs (nur existierende `/de/`- und `/en/`-Seiten, keine Blogs, keine Root-URLs, keine Stubs). Bei neuen Seiten: Sitemap manuell ergänzen, nur kanonische URLs.
6. **hreflang:** x-default zeigt überall auf das `/de/`-Pendant (Marcels Entscheidung: DE ist Standard). Nur echte Übersetzungspaare verknüpfen.
7. **Alte `_redirects`-Datei** nach `_reserve/` verschoben — sie war auf GitHub Pages wirkungslos (Cloudflare-Pages/Netlify-Format).
8. **llms.txt** auf Core Pages reduziert (Blog-Sektionen raus).

## GSC-Status (20.07.2026)

- Sitemap neu eingereicht; Indexierung angefordert für /de/features.html, /de/preis.html, /de/moodle-plugin.html, /de/demos.html.
- Erwartung: Domain-Neubewertung 4-8 Wochen. Wöchentlich Pages-Report prüfen.

## Offen

- portal.alphalearn.ai: `X-Robots-Tag: noindex, nofollow` liegt auf dev, muss auf Production (Kollege). Verifizieren: `curl -I https://portal.alphalearn.ai/`.
